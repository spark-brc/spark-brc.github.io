...
def gantt_chart_gspread(shtname=None, width=None):
    ...
    # And now add those as text in the graph
    text = alt.Chart(newdf, title=f"BLM Watershed Modeling Progress (Gantt) Chart").mark_text(align='left', baseline='middle', dx=3, color="white",  fontWeight="bold").encode(
        y=alt.Y(
            'Tasks',
            # sort=list(df.sort_values(["end date", "start date"])["Tasks"])*2
            sort=None,
    ...
    os.chdir(os.getcwd())
    if shtname is None:
        alt.layer(chart, text).save('index.html', embed_options={'renderer':'svg'})
    else:
        alt.layer(chart, text).save('timeline-{}.html'.format(shtname), embed_options={'renderer':'svg'})
    print('Printed!')
    print(newdf)
