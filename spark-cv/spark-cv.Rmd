...
make_cite <- function(authors,	year,	title,	journal = NULL,	volume = NULL,	
  ...
  if(!is.null(doi) & !is.null(url)) {
    # doi <- paste0("&emsp;&nbsp;<i class='ai fab ai-doi ai-lg'></i>", "[:", doi, "](", url, ")")
    # doi <- paste0('<a href=', url, '><i class="ai ai-doi ai-lg"></i></a>', "[:", doi, "](", url, ")")
    doi <- paste0('[<i class="ai ai-doi ai-lg"></i>',doi,'](',url, ')')
  }

# <- paste0('[<i class="ai ai-doi ai-lg"></i>',doi,'](',url, ')'
#   '<a href=', url, '><i class="ai ai-doi ai-lg"></i></a>', "[:", doi, "](", url, ")")
...
fa_web = ifelse(
  !is.na(submitted$url), 
  paste0(
    '[<i class="fas falink2 fa-globe"></i>](',
    submitted$url,
    ")"),
    "")
